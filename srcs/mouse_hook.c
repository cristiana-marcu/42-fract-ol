...
/*   Updated: 2021/12/06 18:55:34 by cmarcu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	recalculate_viewport(t_complex mouse, t_data *mlx, double scale)
{
	...
	mlx->fractal->viewport.xmin = interpolate(mouse.r, mlx->fractal->viewport.xmin, interpolation);
	mlx->fractal->viewport.xmax = interpolate(mouse.i, mlx->fractal->viewport.xmax, interpolation);
	mlx->fractal->viewport.ymin = interpolate(mouse.r, mlx->fractal->viewport.ymin, interpolation);
	mlx->fractal->viewport.ymax = interpolate(mouse.i, mlx->fractal->viewport.ymax, interpolation);
}
